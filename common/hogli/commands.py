...
def doctor_disk(dry_run: bool, yes: bool) -> None:
    ...
    categories: list[CleanupCategory] = [
        CleanupCategory(
            ...
            description=[
                "Remove Flox CLI logs generated by the dev environment manager.",
                "These logs can grow quickly after repeated 'flox' operations.",
            ],
            estimate=_estimate_flox_logs,
            cleanup=_cleanup_items,
            confirmation_prompt="Clean up Flox logs?",
            dry_run_message="Would delete Flox log files.",
        ),
        CleanupCategory(
        ...
        CleanupCategory(
            ...
            description=[
                "Removes Cargo 'target' directories produced by the Rust services in rust/.",
            ],
        ...
        CleanupCategory(
            ...
            dry_run_message=(
                "Would delete node_modules directories (requires 'pnpm install' afterwards)."
            ),
    ...
    for category in categories:
        results.append(_run_category(category, REPO_ROOT, dry_run, yes))
    ...
    if dry_run:
        ...
        if non_counted:
            ...
            click.echo(
                "   Note: "
                f"{titles} cleanup is not included in the total freed space."
            )
...
def _estimate_flox_logs(repo_root: Path) -> CleanupEstimate:
    """Collect Flox log files under .flox/log."""
    ...
    for log_file in flox_log_dir.glob("*.log"):
        ...
        try:
            ...
            continue
        items.append(CleanupItem(log_file, size, is_dir=False))
        total += size
    ...
    if items:
        details.append(
            f"   Located {len(items)} log file(s) in {relative_dir}."
        )
        details.extend(
            _describe_items(items, repo_root, "   Sample log files:")
        )
    else:
        details.append(f"   No Flox log files present in {relative_dir}.")
...
def _estimate_python_caches(repo_root: Path) -> CleanupEstimate:
    ...
    if items:
        details = [
            f"   Found {len(items)} Python cache director{'ies' if len(items) != 1 else 'y'}."
        ]
        details.extend(
            _describe_items(items, repo_root, "   Sample cache locations:")
        )
    else:
...
def _estimate_dagster_storage(repo_root: Path) -> CleanupEstimate:
    ...
    if items:
        details.append(
            f"   Found {len(items)} file(s) older than 7 days in {storage_dir.relative_to(repo_root)}."
        )
        details.extend(
            _describe_items(items, repo_root, "   Sample files:")
        )
    else:
...
def _estimate_node_artifacts(repo_root: Path) -> CleanupEstimate:
    ...
    if items:
        ...
        details.extend(
            _describe_items(items, repo_root, "   Sample paths:")
        )
    else:
...
def _estimate_rust_targets(repo_root: Path) -> CleanupEstimate:
    ...
    if items:
        ...
        details.extend(
            _describe_items(items, repo_root, "   Sample directories:")
        )
    else:
...
def _collect_paths_from_patterns(
...
def _collect_rust_target_dirs(repo_root: Path) -> list[CleanupItem]:
    """Collect Cargo target directories from rust/ and the repo root."""

    candidates: set[Path] = set()
    rust_root = repo_root / "rust"
    if rust_root.exists():
        candidates.update(_iter_named_directories(rust_root, "target"))

    root_target = repo_root / "target"
    if root_target.exists():
        candidates.add(root_target)
    ...
    for candidate in sorted(candidates):
        ...
        if size <= 0:
            continue
        items.append(CleanupItem(candidate, size, is_dir=True))
...
def _describe_items(
    items: Sequence[CleanupItem], repo_root: Path, heading: str
) -> list[str]:
    ...
    for item in sorted(items, key=lambda entry: entry.path)[:MAX_SAMPLE_PATHS]:
        try:
            ...
            relative = item.path
        lines.append(
            f"     - {relative} ({_format_size(item.size)})"
        )
